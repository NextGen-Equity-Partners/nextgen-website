      ...
      <div className="tm-body">
        ...
        {member.linkedin && (
          <a
            ...
            aria-label={`${member.name} ${linkedinSuffix}`}
          >
...
export default function Page() {
  ...
  const linkedinSuffix = tr.nav.linkedinOf[locale];
  return (
    <>
        ...
        <div className="pane-inner">
          ...
          <div className="team-grid">
            <div className="team-row-3">
              {c.partners.map((m, i) => (
                <TeamCard key={m.name} member={m} delay={(i + 1) as 1 | 2 | 3} linkedinSuffix={linkedinSuffix} />
              ))}
            </div>
            <div className="team-row-2">
              {c.extended.map((m, i) => (
                <TeamCard key={m.name} member={m} delay={(i + 1) as 1 | 2} linkedinSuffix={linkedinSuffix} />
              ))}
            </div>
          </div>
        </div>
